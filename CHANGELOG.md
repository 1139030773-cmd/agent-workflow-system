...
### Added
- 🛡️ **对抗验证** — 铁律新增第 6 步：spawn 独立 agent 在发布前语义验证"应该改但没改"的遗漏，打破 AI 自我确认盲区

### Changed
- 🔧 **铁律 7 步 → 8 步** — 原第 6 步（commit+push+release）→ 第 7 步，原第 7 步（版本一致性验证）→ 第 8 步

## [1.12.0] - 2026-06-12

### Added
- 🔍 **调研闸门** — 新手引导新增可选调研环节：产品/创业类项目在方向明确后，询问用户是否需要背景调研
- 📊 **调研简报字段** — briefing artifact 新增 `调研简报` 区块
- 📏 **交互预算规则** — 新增 `INTERACTION_BUDGET.md`：通用 ≤1 决策点 + 调研同步场景豁免（≤2，串行触发）
- ✅ **建议溯源** — BEHAVIOR_SPEC 新增第十五章：所有面向用户的建议和判断必须有可追溯依据

### Changed
- 🔧 **newbie-guide** — 第三步后插入调研闸门，第六步前插入调研同步步骤
- 🔧 **project-master** — 开始前读取调研简报，决策闸门中引用外部调研发现

## [1.11.1] - 2026-06-12

### Fixed
- 🐛 **health-check.ps1 编码修复** — 脚本加 UTF-8 BOM + Get-Content 统一 -Encoding UTF8 + 多行正则 `$` 锚点修复
